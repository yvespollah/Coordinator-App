...
const Home = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchManagersCount().catch(() => 0),
      fetchVolunteersCount().catch(() => 0),
      fetchWorkflowsCount().catch(() => 0),
      fetchTasksCount().catch(() => 0)
    ...
    Promise.all([
      fetchWorkflowsByStatus().catch(() => ({
        CREATED: 2,
        RUNNING: 3,
        COMPLETED: 5,
        FAILED: 1
      })),
      fetchVolunteersByStatus().catch(() => ({
        available: 3,
        busy: 2,
        offline: 1
      }))
